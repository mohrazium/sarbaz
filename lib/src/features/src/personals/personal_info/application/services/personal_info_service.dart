...
import '../../../contacts/application.dart';
...
class PersonalInfoServiceImpl implements PersonalInfoService {
  ...
  Future<int> save(PersonalInfoModel e) async {
    return await _personalInfoDAO.doInsert(e.toJson()).then((value) {
      logger.info("person is saved");
      return value.id ?? 0;
    }).onError((error, stackTrace) => throw FailureException("saving personal info failed with error $stackTrace"));
  }
  ...
  Future<List<PersonalInfoModel>> findAll() async {
    ...
    await _personalInfoDAO.findAll().then((dataList) async {
      models.clear();
      for (var data in dataList) {
        ...
        if (data.contactInfo != null) {
          contactInfoModel = await Get.find<ContactInfoService>().findById(data.contactInfo!);
          personalInfoModel = personalInfoModel.copyWith(contactInfo: contactInfoModel);
        }

        if (data.soldier != null) {
          soldierModel = await Get.find<SoldierService>().findById(data.soldier!);
          personalInfoModel = personalInfoModel.copyWith(soldier: soldierModel);
        }
        models.add(personalInfoModel);
      }
    }).onError(
        (error, stackTrace) => throw FailureException("loading all personal infos, failed with error $stackTrace"));
    return models;
  }
  ...
  Future<PersonalInfoModel?> findById(int id) async {
    return await _personalInfoDAO.findById(id).then((value) {
      return value != null ? PersonalInfoModel.fromJson(value.toJson()) : null;
        ...
        throw FailureException('An error happened on find by id in $stackTrace.', exception: ExceptionType.NOT_FOUND));
  }

  @override
  Future<bool> update(PersonalInfoModel e) async {
    return await _personalInfoDAO.doUpdate(e.toJson()).then((value) => value).onError(
        (error, stackTrace) => throw FailureException("updating personal info failed, with error $stackTrace"));
  }

  @override
  Future<bool> delete(PersonalInfoModel e) async {
    return await _personalInfoDAO.doDelete(e.toJson()).then((value) => value > 0).onError((error, stackTrace) =>
        throw FailureException("deleting personal info failed, with error:\n $error \n $stackTrace'"));
  }

  @override
  Future<PersonalInfoModel?> findByNationalCode(String nationalCode) async {
    return await _personalInfoDAO.findByNationalCode(nationalCode).then((value) {
      return value != null ? PersonalInfoModel.fromJson(value.toJson()) : null;
  ...
  @override
  Future<int> findFurtherInfoIdById(int personalId) {
    return _personalInfoDAO.findById(personalId).then((value) {
      return value != null && value.furtherInfo != null ? value.furtherInfo! : 0;
    }).onError((error, stackTrace) =>
        throw FailureException("personal info not found by id, see the error :\n $error \n $stackTrace"));
  }

  @override
  Future<int> findContactInfoIdById(int personalId) {
    return _personalInfoDAO.findById(personalId).then((value) {
      return value != null && value.contactInfo != null ? value.contactInfo! : 0;
    }).onError((error, stackTrace) =>
        throw FailureException("personal info not found by id, see the error :\n $error \n $stackTrace"));
  }
  ...
  Future<int> findEducationalInfoIdById(int personalId) {
    return _personalInfoDAO.findById(personalId).then((value) {
      return value != null && value.educationalInfo != null ? value.educationalInfo! : 0;
    }).onError((error, stackTrace) =>
        throw FailureException("personal info not found by id, see the error :\n $error \n $stackTrace"));
  }
  ...
  Future<bool> deleteById(int id) async {
    final foundedPerson = await findById(id);
    return delete(foundedPerson!).onError((error, stackTrace) => throw FailureException("$error \n $stackTrace"));
  }
}
