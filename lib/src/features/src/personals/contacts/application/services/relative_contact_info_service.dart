...
abstract class RelativeContactsInfoService extends Service<int, RelativeContactsInfoModel> {
...
class RelativeContactsInfoServiceImpl implements RelativeContactsInfoService {
  ...
  Future<bool> delete(RelativeContactsInfoModel model) async {
    return await dao.doDelete(model.toJson()).then((value) => value == 1).onError(
        (error, stackTrace) => throw FailureException("deleting relative contact info failed, with error $stackTrace"));
  }
  ...
  Future<int> save(RelativeContactsInfoModel model) async {
    if (model.contactInfo != null) {
      return await dao.doInsert(model.toJson(), model.contactInfo!.id!).then((value) {
        logger.info("Relative contact saved by contact id, ${model.phoneNumber}");
        return value.id ?? 0;
      }).onError(
          (error, stackTrace) => throw FailureException("Relative contact can not save by contact id. $stackTrace"));
    } else {
  ...
  Future<bool> update(RelativeContactsInfoModel model) async {
    return await dao.doUpdate(model.toJson()).then((value) => value).onError(
        (error, stackTrace) => throw FailureException("updating relative contact info failed, with error $stackTrace"));
  }

  @override
  Future<List<RelativeContactsInfoModel>?> findAllByContactId(int contactId) async {
    ...
    return await dao.findAllByContactId(contactId).then((values) {
      if (values.isNotEmpty) {
        ...
        throw FailureException(" Can not retrieve relative contact from id : $contactId with error : $stackTrace "));
  }
}
