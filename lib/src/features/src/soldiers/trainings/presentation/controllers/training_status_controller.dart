...
import 'package:sarbaz/src/config/src/logger/logger_service.dart';
...
class TrainingStatusController extends GetxController with ValidatorMixin, DateConverterMixin {
  ...
  TrainingStatusController(this._trainingStatusService, this._baseController);
  ...
  Future<void> onConfirmButtonPressed() async {
    if (!readOnly.value) {
      await _baseController.isSoldierSaved().then((value) {
        if (value) {
  ...
  void onCalenderPressed(context, isStart) async {
    ...
    if (picked != null) {
      if (isStart) {
        startDateController.text = convertEnToFa(picked.formatCompactDate());
      } else {
        endDateController.text = convertEnToFa(picked.formatCompactDate());
      }
  ...
  Future<void> _loadData() async {
    await _trainingStatusService.findByPersonalInfoId(_baseController.personalInfoId.value).then((value) {
      if (value?.id != null) {
        _clearEditor();
        readOnly(true);
        model(value);
