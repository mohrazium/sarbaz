...
class SoldiersController extends GetxController {
  ...
  SoldiersController(this._personalInfoService, this._baseController);
  ...
  Future<void> _removeSoldier() async {
    int id = int.parse(
        soldiersDataSource.value.dataGridRows[cellTapDetails.rowColumnIndex.rowIndex - 1].getCells().last.value);
    await Get.find<PersonalInfoService>().deleteById(id).then((isDeleted) {
      if (isDeleted) {
        showToast(Strings.successfullyDeleted);
        _loadAll();
        searchSoldier(null);
        _baseController.initSoldierEditorForms(0,withLoading:true);
      }
    }).catchError((onError) {
      DialogHelper.showCrashReport(onError.toString());
    });
  ...
  void onCellDoubleTap(DataGridCellDoubleTapDetails details) {
    int id =
        int.parse(soldiersDataSource.value.dataGridRows[details.rowColumnIndex.rowIndex - 1].getCells().last.value);
    _baseController.setDashboardTab(_baseController.dashboardTabSoldiersEditor);
    _baseController.initSoldierEditorForms(id,withLoading:true);
  }
  ...
  void loadAllSoldiers([bool withLoading = false]) {
    _loadAll();
    searchSoldier(null);
    if (_baseController.personalInfoId.value != 0) {
      _baseController.initSoldierEditorForms(_baseController.personalInfoId.value, withLoading: withLoading);
    }
  }
}
