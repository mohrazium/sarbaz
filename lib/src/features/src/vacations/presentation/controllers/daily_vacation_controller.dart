...
class DailyVacationController extends GetxController with ValidatorMixin, DateConverterMixin {
  ...
  DailyVacationController(this._baseController, this._dailyVacationService);
  ...
  Future<void> onConfirmButtonPressed() async {
    if (!readOnly.value) {
      await _baseController.isSoldierCaseSaved().then((value) {
        if (value) {
  ...
  void _save() async {
    ...
    if (model.value.id == null) {
      model(model.value
          .copyWith(vacations: VacationsModel.init().copyWith(id: await _baseController.getCurrentVacationsId())));
      await _dailyVacationService
          .saveByPersonalInfoId(model.value, _baseController.personalInfoId.value)
          .then((value) {
        if (value != null && value.id! != 0) {
          readOnly(true);
          model(model.value.copyWith(id: value.id));
          _baseController.dailyVacationId(value.id);
          _loadData();
      ...
      _dailyVacationService
          .updateByPersonalInfoId(model.value, _baseController.personalInfoId.value)
          .then((isUpdated) {
        if (isUpdated) {
          readOnly(true);
          _loadData();
  ...
  Future<void> _loadData() async {
    await _dailyVacationService
        .findById(model.value.id ?? _baseController.dailyVacationId.value)
  ...
  Future<String?> _getDateFromPicker(ctx) async {
    ...
    if (picked != null) {
      return convertEnToFa(picked.formatCompactDate());
    }
    return null;
  }
