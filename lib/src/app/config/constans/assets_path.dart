
class FontConstants {
  static const sahel = 'Sahel';
  static const vazir = 'Vazir';
}

class ImageAnimationPath {
  // you can get free animation image from rive or lottiefiles

  // Example:
  // static const _folderPath = "assets/images/animation";
  // static const myAnim = "$_folderPath/my_anim.json";
}

class ImagesConstants {
  static const _folderPath = "assets/images";
  static const avatar = "$_folderPath/man.png";
  static const Map<int, String> ranksMarkImage = {
    1: "$_folderPath/01.png",
    2: "$_folderPath/02.png",
    3: "$_folderPath/03.png",
    4: "$_folderPath/04.png",
    5: "$_folderPath/05.png",
    6: "$_folderPath/06.png",
    7: "$_folderPath/07.png",
    8: "$_folderPath/08.png",
    9: "$_folderPath/09.png",
    10: "$_folderPath/10.png",
    11: "$_folderPath/11.png",
    12: "$_folderPath/12.png",
  };
}

class VectorImagesConstants {
  // Example:
  // static const _folderPath = "assets/images/vector";
  // static const myVector = "$_folderPath/vector/my_vector.svg";
}
