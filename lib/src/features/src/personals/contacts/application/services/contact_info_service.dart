...
abstract class ContactInfoService extends Service<int, ContactInfoModel> {
  Future<int> saveWithParentId(ContactInfoModel model, {required int personalInfoId});
...
class ContactInfoServiceImpl implements ContactInfoService {
  ...
  Future<bool> update(ContactInfoModel model) async {
    return await dao.doUpdate(model.toJson()).then((value) => value).onError((error, stackTrace) =>
        throw FailureException("Updating contact info failed, see the error :\n $error \n $stackTrace"));
  }

  @override
  Future<ContactInfoModel?> findByPersonalInfoId(int personalInfoId) async {
    return await dao
        .findById(await Get.find<PersonalInfoService>().findContactInfoIdById(personalInfoId))
        .then((value) {
      return value != null ? ContactInfoModel.fromJson(value.toJson()) : null;
    }).onError((error, stackTrace) => throw FailureException(
  ...
  @override
  Future<int> saveWithParentId(ContactInfoModel model, {required int personalInfoId}) async {
    int res = 0;
    await dao.doInsert(model.toJson(), personalInfoId).then((value) {
      res = value.id ?? 0;
    }).onError((error, stackTrace) =>
        throw FailureException("Contact info can not save, see the error :\n $error \n $stackTrace"));
    return res;
  }
}
